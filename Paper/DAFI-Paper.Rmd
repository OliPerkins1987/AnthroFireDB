...
Overall, 20 anthropogenic fire uses were identified during literature review, but many were closely related (e.g., ‘pasture renewal’ and ‘rangeland management’). After such similar types were combined (see Appendix C), seven dominant fire uses emerged (Table 2) each with more than 100 instances in the database and accounting for `r fu.contrib`% of fire instance records.
...
Similarly, fire return intervals are typically short, with median of `r ri.med` and mean of `r ri.mean` years.
...
We find that `r round(perc21,2)`% of records in DAFI for mean size of deliberately started fires are <21 ha, suggesting many anthropogenic fires will not be detected. 

Ramo et al. (2021) also found that fires <100 ha are critically important for characterizing landscape fire on a global scale. Our results from DAFI support this; `r round(perc100,2)`% of mean fire size records for deliberately started fires are <100ha, and cropland fires in pre-industrial and transition regimes are generally smaller compared to fires ignited to burn across landscapes more broadly (Figure 3x!).

Furthermore, the median and mean ignition density for deliberate anthropogenic fires in DAFI (`r igs.med` and `r igs.mean` respectively, section 3.2.2) are far greater than the median value suggested by the MODIS-derived global fire atlas (< 0.01 fires km-2 year-1; Andela et al., 2019).
...
```
In particular, hunter-gatherer fire sizes vary more than pasture managament fires, with sizes ranging from `r round(hg.min,1)` ha to `r round(hg.max,1)` ha compared to `r round(pas.min,1)` ha to `r round(pas.max,1)` ha.
