...
DB_overview <- function(simp = T, AFT.eval = F) {
  ...
  if(simp == T) {
    ...
    Simp.fire$`Fire purpose`   <- Simplify.purpose()
    Simp.est                   <- est_fire
    Simp.est$`Fire purpose`    <- Simplify.purpose()
  ...
  if(AFT.eval == F) {
    ...
    N            <- Simp.fire[, -which(colnames(Simp.fire) %in% c(
      "Actual burned area (ha)", "Intended burned area (ha)"))] %>% group_by(`Fire purpose`) %>% 
      filter(`Presence / Absence` == 'Presence') %>% summarise(Instances = n())
    
    
    metrics      <- Simp.fire[, -which(colnames(Simp.fire) %in% c("Actual burned area (ha)", "Intended burned area (ha)", "Study Year"))] %>% group_by(`Fire purpose`) %>% 
      summarise_if(is.numeric, function(x) {length(which(!is.na(x) & x != 0))}) 
    
    est_metrics  <- Simp.est %>% group_by(`Fire purpose`) %>% 
    ...
    both_metrics           <- both_metrics[, c(ncol(both_metrics), 1:ncol(both_metrics)-1)]
    both_metrics$instances <- c(N$Instances[1:6], 0, N$Instances[7:27])
    ...
    AFT.actions <- plyr::rbind.fill(Simp.fire %>%
                                      ...
                                      lapply(function(y) {data.frame(table(y$`Fire purpose`), 
                                                                     rep(y$AFTname[1], times = length(unique(y$`Fire purpose`))))}))
