...
load.db <- function(r.notes = TRUE, env = .GlobalEnv) {
  ...
  colnames(est_fire)              <- gsub(' ', '.', colnames(est_fire))
...
DB_overview <- function(simp = T, AFT.eval = F) {
  ...
  if(simp == T) {
    ...
    Simp.fire$Fire.intention <- ifelse(Simp.fire$Fire.intention %in% c('Pasture renewal', 'Rangeland management'), 'Pasture management', 
                                  ifelse(Simp.fire$Fire.intention %in% c('Pest management') & Simp.fire$AFT %in% c('Cattle, Extensive', 'Pastoralism, Migratory'), 'Pasture management',
                                  ifelse(Simp.fire$Fire.intention %in% c('Forest clearance', 'Land clearance'), 'Primary vegetation clearance',
                                  ifelse(Simp.fire$Fire.intention %in% c('Hunting', 'Fishing', 'Harvesting of NTFP'), 'Hunting and gathering', 
                                  ifelse(Simp.fire$Fire.intention %in% c('Pyrome management', 'Conservation'), 'Pyrome management', 
                                  ifelse(Simp.fire$Fire.intention %in% c('Accessibility', 'Charcoal production', 'Cultural & Spiritual', 'Vegetation management', 'Forest management'), 'Other', 
                                  ifelse(Simp.fire$Fire.intention %in% c('All', 'ND'), NA, Simp.fire$Fire.intention)))))))
    
    Simp.est                 <- est_fire
    Simp.est$Fire.intention  <- Simp.fire$Fire.intention
  ...
  if(AFT.eval == F) {
    ...
    if(simp == T) {
      ...
      Simp.fire <- reported_fire %>% filter(!AFT %in% c('Abandoned agricultural land', 
                                                        'Agroforestry, Market-oriented', 'Agroforestry, Subsistence-oriented', 
                                                        'Cropping, Agroecology', 'Unoccupied', 'All') & !is.na(Fire.intention))
      
      Simp.est  <- est_fire %>% filter(!AFT %in% c('Abandoned agricultural land', 
                                                   'Agroforestry, Market-oriented', 'Agroforestry, Subsistence-oriented', 
                                                   'Cropping, Agroecology', 'Unoccupied', 'All') & !is.na(Fire.intention))
    ...
    Simp.fire   <- Simp.fire %>% 
      filter(Fire.type %in% c('Human, deliberate', 'Human, escaped') & Fire.intention != 'ND') 
    ...
    AFT.actions  <- tidyr::pivot_wider(AFT.actions, names_from = "rep.y.AFTname.1...times...length.unique.y.Fire.intention...", 
                                       values_from = "Freq")
