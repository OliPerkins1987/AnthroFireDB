...
estimate.metrics <- function(dat, metric = 'size', kind = 'estimated') {
  ...
  if(metric == 'size') {
    ...
    dat <- dat[apply(dat[, 11:18], 1, function(x) {length(which(!is.na(x)))}) >= 2, ]
    dat <- dat[dat$`Presence./.Absence` == 'Presence', ]
    
  }
  
  est_dat <- data.frame('Case.Study.ID' = NA, 'AFT' = '', 'Fire intention' = '', 'Reported.Firemetric.1' = '', 
    ...
    for(i in 1:nrow(dat)){
      ...
      if(metric == 'size') {
        ...
        for (col in 1:length(key)) {
          ...
          est_dat[nrow(est_dat), 3+col] <- key[col]
        ...
        has.mean   <- any(TRUE %in% grepl('mean', est_dat[i, 4:7]))
        has.median <- any(TRUE %in% grepl('median', est_dat[i, 4:7]))
        has.min    <- any(TRUE %in% grepl('min', est_dat[i, 4:7]))
        has.max    <- any(TRUE %in% grepl('max', est_dat[i, 4:7]))
        ...
        if(has.mean & !has.median) {
          ...
          est_dat$Est.metric[nrow(est_dat)] <- paste(dat.kind, 'median', sep = '.')
          lambda                            <- 1/mean(as.numeric(c(sub("\\-.*", "", ifelse(dat.kind =='Intended', dat$`Intended.fire.size.mean.(ha)`[i], dat$`Actual.fire.size.mean.(ha)`[i])), 
                                                                   sub("*.-", "", ifelse(dat.kind =='Intended', dat$`Intended.fire.size.mean.(ha)`[i], dat$`Actual.fire.size.mean.(ha)`[i])))))
          
          
          est_dat$Est.value[nrow(est_dat)]  <- log(2) / lambda
        ...
        if(has.median & !has.mean) {
          ...
          est_dat$Est.metric[nrow(est_dat)] <- paste(dat.kind, 'mean', sep = '.')
          ...
          est_dat$Est.value[nrow(est_dat)]  <- 1 / lambda
